...
static int gtp_power_switch(struct i2c_client *client, int on)
{
    ...
    /*
     * ATK-7RGBLCD 触摸模组供电来自屏座，dts 通常不写 vdd_ana/vcc_i2c。
     * 无对应 *-supply 时 regulator_get 失败属正常，跳过即可（勿让 probe 失败）。
     */
    if (skip_regulator)
        return 0;
    ...
    if (!vcc_i2c) {
        ...
        if (IS_ERR(vcc_i2c)) {
            GTP_INFO("no vcc_i2c supply in DT, skip regulator control");
