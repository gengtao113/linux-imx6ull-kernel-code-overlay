...
> 关联：`04`（原理图）、`17`（问题复盘）、`18`（匹配机制与调用链）、`19`/`20`（SPI 子系统架构与框架笔记）、`origin/IMX6`（出厂 dts 片段）
...
行末 `0x100b1` / `0x100b0` 是 **PAD_CTL 电气参数**（非 ALT 号）；位域拆解见文档 **`18` §二 pinctrl 小节**。
